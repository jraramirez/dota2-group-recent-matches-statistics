...
def calculateMeteorHammerPurchases(matches):
    ...
    for match in matches:
        ...
        if df.matchHasMeteorHammer(matchData):
            ...
            for player in players:
                if player["account_id"] in defaults.ACCOUNT_IDS:
                    meteorHammerPlayerPurchases[defaults.ACCOUNT_IDS.index(player["account_id"])]["number_of_matches"] += 1
    ...
    numberOfLosses = len(matches) - numberOfWins
